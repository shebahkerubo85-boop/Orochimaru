...
class GogoAnimeProvider : NativeAnimeParser() {
    ...
    private fun parseEpisodes(
        ...
        epRegex.findAll(body).forEach { match ->
            ...
            episodes.add(Episode(
                ...
                link = "$baseUrl/$animeLink-episode-$number-english-subbed/",
    ...
    override suspend fun loadVideoServers(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val serverBlockRegex = Regex(
                    """<div class="type" data-type="([^"]+)">(.*?)</div>""",
                    setOf(RegexOption.IGNORE_CASE, RegexOption.DOT_MATCHES_ALL),
                )
                val itemRegex = Regex(
                    """<li\s+class="player-type-link"\s+data-src="([^"]+)"[^>]*>(.*?)</li>""",
                    setOf(RegexOption.IGNORE_CASE, RegexOption.DOT_MATCHES_ALL),
                )

                serverBlockRegex.findAll(body).forEach { block ->
                    val blockLanguage = block.groupValues[1].uppercase()
                    if (blockLanguage !in languages) return@forEach
                    itemRegex.findAll(block.groupValues[2]).forEach { item ->
                        val playerUrl = attr(item.value, "data-src")
                            .ifBlank { item.groupValues[1].replace("&amp;", "&") }
                            .replace("&amp;", "&")
                        if (!seen.add(playerUrl)) return@forEach
    ...
    private fun videoServer(namedStream: NamedStream): VideoServer {
        val stream = namedStream.stream
        val extraData = mutableMapOf<String, String>(
            "format" to stream.format.name,
            "referer" to stream.headers["Referer"].orEmpty(),
        )
        stream.quality?.let { extraData["quality"] = it.toString() }
        if (stream.subtitles.isNotEmpty()) {
            extraData["subtitles"] = JsonArray(stream.subtitles.map { subtitle ->
                buildJsonObject {
        ...
        return VideoServer(
            namedStream.name,
            ani.sanin.FileUrl(stream.url, stream.headers),
            extraData,
        )
    }
}
